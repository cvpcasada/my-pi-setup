...
const HIGHLIGHT_THEME_NAMES = {
  dark: "vesper",
  light: "pierre-light",
} as const;

const HIGHLIGHT_RENDER_OPTIONS = {
  dark: {
    theme: HIGHLIGHT_THEME_NAMES.dark,
  ...
  light: {
    theme: HIGHLIGHT_THEME_NAMES.light,
    useTokenTransformer: false,
    tokenizeMaxLineLength: 1000,
    lineDiffType: "word-alt" as const,
    maxLineDiffLength: 2000,
  ...
  try {
    ...
    const highlighterOptions =
      ...
      getHighlighterOptions(language, {
        theme: HIGHLIGHT_THEME_NAMES[appearance],
      });
    ...
    const highlighted = renderDiffWithHighlighter(
      metadata,
      highlighter,
      HIGHLIGHT_RENDER_OPTIONS[appearance],
    );
